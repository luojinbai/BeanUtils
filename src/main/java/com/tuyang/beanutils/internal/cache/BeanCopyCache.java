...
import com.tuyang.beanutils.internal.convertors.*;
            ...
            if (propertyField != null && propertyField.isAnnotationPresent(CopyProperty.class)) {
                ...
                if (convertorClass != null) {
                    ...
                    if (PropertyUtils.isAssignable(methodTargetType, methodSourceType)) {
                        ...
                        BeanCopyPropertyItem item = new BeanCopyPropertyItem();
                        // 如果是 int 转 enum， 通过 valueOf方法转
                        if (methodSourceType.equals(Integer.class) || methodSourceType.equals(int.class)) {
                            item.propertyName = propertyName;
                            item.writeMethod = writeMethod;
                            item.readMethods = readMethods;
                            item.isCollection = false;
                            item.optionClass = null;
                            item.convertorClass = IntToEnumConvertor.class;
                            item.convertorObject = new IntToEnumConvertor(methodTargetType);
                            item.useBeanCopy = true;
                        ...
                        // enum 转 string， 通过enum.toString()转
                        BeanCopyPropertyItem item = new BeanCopyPropertyItem();
                        ...
                        item.isCollection = false;
                        item.optionClass = null;
                        item.convertorClass = EnumToIntConvertor.class;
                        item.convertorObject = new EnumToIntConvertor(methodSourceType);
                        item.useBeanCopy = true;
